...
const collectionPath: string = program.opts().collection;
...
function isItem(itemG: Item | ItemGroup): itemG is Item {
  return (<Item>itemG).request !== undefined;
}

let environmentVariables = new VariableList(
  new Property({name: 'environmentVariables'}),
  [],
);
if (program.opts().envvars) {
  const environment = JSON.parse(
    readFileSync(program.opts().envvars).toString(),
  );
...
const lvp: {language: string; variant: string} =
  program.opts().language_variant;
debugPrint(environmentVariables);
collection.items.all().forEach((item: any) => {
  if (isItem(item)) {
    codegen.convert(
      lvp.language,
      lvp.variant,
      ...
      function (error: any, snippet: any) {
        ...
        const completeSnippet = environmentVariables.replace(snippet);
        const re = /(?:\{\{(.+?)\}\})/g;
